# Gadget Factory — the store app

One app, one game. This folder wraps the web game in `games/gadget-factory` unchanged
...
- **App id:** `com.kentbuhr.gadgetfactory`. Change it in `capacitor.config.json` before the first
...
- **The bonus shop** plays a timed stand-in for a rewarded ad and says so on
  ...
  store builds.
- **Store assets** are in `assets/` once `node tools/make-store-assets.mjs gadget-factory`
